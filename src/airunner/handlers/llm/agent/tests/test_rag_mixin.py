...
def test_index_refresh_adds_new_nodes(monkeypatch, rag_mixin_with_mocks):
    ...
    doc2 = MagicMock()
    doc2.doc_id = "doc2"
    with patch.object(RAGMixin, "documents", new_callable=PropertyMock) as mock_docs:
        ...
        rag.text_splitter.get_nodes_from_documents.side_effect = lambda docs: (
            [mock_node] if docs and getattr(docs[0], "doc_id", None) == "doc2" else []
        )
    ...
    # Debug output
    print("add_documents call args:", mock_index.docstore.add_documents.call_args)
    print("index_struct.table:", mock_index.index_struct.table)
    # Should add new node to docstore
    mock_index.docstore.add_documents.assert_called_with([mock_node], allow_update=True)
