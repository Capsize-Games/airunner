...
class TestRAGPerformance(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.rag_mixin._extract_keywords_from_text = (
            RAGMixin._extract_keywords_from_text
        )
        ...
        self.test_documents = [
            Document(
                text=f"This is a test document {i} for RAG testing with various keywords like AI, machine learning, and data science.",
                doc_id=f"doc_{i}",
            )
            for i in range(10)
        ]
    ...
    def test_keyword_extraction_performance(self):
        ...
        print(
            f"First run time: {first_run_time:.4f}s, Second run time: {second_run_time:.4f}s"
        )
    ...
    def test_extract_keywords_correctness(self):
        ...
        self.assertTrue(
            "artificial" in all_words
            or "intelligence" in all_words
            or "machine" in all_words
            or "learning" in all_words
            or "transforming" in all_words
            or "industries" in all_words
        )
