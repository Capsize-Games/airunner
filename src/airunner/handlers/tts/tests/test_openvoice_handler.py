...
class TestOpenVoiceHandler(unittest.TestCase):
    def setUp(self):
        self.mock_settings = MagicMock()
        self.mock_settings.tts_model_path = "mock_path"
        self.mock_settings.speaker_recording_path = "mock_speaker_path"

        with patch("airunner.handlers.tts.openvoice_handler.se_extractor.get_se", return_value=(MagicMock(), "mock_audio_name")):
            self.handler = OpenVoiceHandler(
                tts_settings=self.mock_settings,
                path_settings=MagicMock()
            )

    @patch("airunner.handlers.tts.openvoice_handler.TTS")
    ...
    @patch("airunner.handlers.tts.openvoice_handler.TTS")
    @patch("airunner.handlers.tts.openvoice_handler.StreamingToneColorConverter.convert", return_value="mock_audio")
