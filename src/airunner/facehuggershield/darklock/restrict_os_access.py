...
class RestrictOSAccess(metaclass=Singleton):
    ...
    def activate(self, whitelisted_directories=None, whitelisted_modules=None, allow_network=False):
        logger.debug(f"RestrictOSAccess.activate called. Current sys._airunner_os_restriction_activated: {getattr(sys, '_airunner_os_restriction_activated', False)}")
        ...
        self.whitelisted_directories = [os.path.abspath(os.path.normpath(d)) for d in whitelisted_directories] if whitelisted_directories else []
        self.whitelisted_modules = set(whitelisted_modules) if whitelisted_modules else set()
        self.allow_network = allow_network
        logger.debug(f"Set self.whitelisted_directories to: {self.whitelisted_directories}")
        ...
        if hasattr(sys, '_airunner_os_restriction_activated') and sys._airunner_os_restriction_activated:
            logger.warning("OS restriction was already activated. Whitelists and settings have been updated.")
            return
    ...
    def deactivate(self):
        if not hasattr(sys, '_airunner_os_restriction_activated') or not sys._airunner_os_restriction_activated:
    ...
    def is_path_whitelisted(self, file_path: str) -> bool:
        ...
        logger.debug(f"is_path_whitelisted: Checking directory '{directory}' for file '{abs_file_path}'")
    ...
    def is_directory_whitelisted(self, directory_path: str) -> bool:
        if not directory_path: # Check for None or empty string
            logger.warning("is_directory_whitelisted called with empty or None directory_path.")
            return False

        abs_path_to_check = os.path.abspath(os.path.normpath(directory_path))
        logger.debug(f"is_directory_whitelisted: Checking absolute path '{abs_path_to_check}'")
        ...
        for whitelisted_dir_processed in self.whitelisted_directories:
            ...
            logger.debug(f"Comparing '{abs_path_to_check}' with whitelisted_dir '{whitelisted_dir_processed}'")
            ...
            if whitelisted_dir_processed == os.sep: # Whitelisted directory is the root '/'
                # Any absolute path starts with root. This effectively whitelists everything if '/' is given.
                if abs_path_to_check.startswith(whitelisted_dir_processed): # Should always be true for absolute paths
                    logger.debug(f"Path '{abs_path_to_check}' is whitelisted via root '{whitelisted_dir_processed}'.")
                    return True
            elif abs_path_to_check.startswith(whitelisted_dir_processed + os.sep):
                logger.debug(f"Path '{abs_path_to_check}' is whitelisted as subdirectory of '{whitelisted_dir_processed}'.")
                return True
                
        logger.debug(f"Path '{abs_path_to_check}' is NOT whitelisted after checking all rules.")
        return False

    def is_file_operation_allowed(self, path: str, operation: str) -> bool:
        logger.debug(f"is_file_operation_allowed: Checking file operation '{operation}' for path '{path}'")
        if not path:
            logger.warning("is_file_operation_allowed called with empty or None path.")
            return False
        abs_path = os.path.abspath(os.path.normpath(path))
        logger.debug(f"is_file_operation_allowed: Normalized absolute path '{abs_path}'")
        ...
        logger.debug(f"is_file_operation_allowed: Path '{abs_path}' whitelisted: {allowed}")
        return allowed
    ...
    def restricted_open(self, file, mode='r', buffering=-1, encoding=None, errors=None, newline=None, closefd=True, opener=None):
        ...
        if isinstance(file, int): # File descriptor
            ...
            try:
                ...
                logger.error(f"Cannot decode bytes path for open: {file}. Error: {e}. Denying access.")
                raise PermissionError(f"File system open operation with non-decodable bytes path '{file!r}' is not allowed.") from e
        else:
            file_path_str = str(file)

        abs_file_path = os.path.abspath(os.path.normpath(file_path_str))
        logger.debug(f"Checking whitelist for absolute file path: '{abs_file_path}' (original: '{file_path_str}')")

        if self.is_path_whitelisted(abs_file_path):
            logger.info(f"Path '{abs_file_path}' is whitelisted for open. Proceeding.")
            return self.original_open(file, mode, buffering, encoding, errors, newline, closefd, opener)
        else:
            logger.error(f"File system open operation to '{abs_file_path}' (from original path '{file}') is not allowed.")
            raise PermissionError(f"File system open operation to '{abs_file_path}' (from original path '{file}') is not allowed.")
    ...
    def restricted_os_write(self, fd, data):
        ...
        logger.debug(f"restricted_os_write called for fd: {fd}. Allowing by default (relies on secure open).")
        return self.original_os_write(fd, data)

    def restricted_makedirs(self, name, mode=0o777, exist_ok=False):
        logger.debug(f"restricted_makedirs called for path: '{name}', mode: {oct(mode)}, exist_ok: {exist_ok}")
        abs_target_path = os.path.abspath(os.path.normpath(name))
        logger.debug(f"Normalized absolute target path for makedirs: '{abs_target_path}'")
        ...
        if exist_ok and os.path.exists(abs_target_path):
            if os.path.isdir(abs_target_path):
                ...
                if self.is_directory_whitelisted(abs_target_path):
                    logger.info(f"Makedirs: Path '{abs_target_path}' exists, is whitelisted, and exist_ok=True. No operation needed.")
                    return None 
                else:
                    logger.error(f"Makedirs: Path '{abs_target_path}' exists and exist_ok=True, but it's NOT whitelisted. Denying.")
                    raise PermissionError(f"File system makedirs operation on existing but non-whitelisted path '{abs_target_path}' is not allowed (exist_ok=True).")
                ...
                logger.warning(f"Makedirs: Path '{abs_target_path}' exists but is not a directory. Original os.makedirs would likely error.")
                if not self.is_directory_whitelisted(abs_target_path): # Check if the location is permissible for creation
                     logger.error(f"Makedirs: Path '{abs_target_path}' (existing non-directory) is also not in a whitelisted location. Denying.")
                     raise PermissionError(f"File system makedirs operation on '{abs_target_path}' (existing non-directory) is not allowed as its location is not whitelisted.")
                logger.debug(f"Makedirs: Path '{abs_target_path}' (existing non-directory) is in a whitelisted location. Letting original_makedirs handle.")
        ...
        if self.is_directory_whitelisted(abs_target_path):
            logger.info(f"Makedirs: Path '{abs_target_path}' is whitelisted for creation. Calling original os.makedirs.")
            try:
                return self.original_makedirs(name, mode, exist_ok=exist_ok)
            except Exception as e:
                logger.error(f"Original os.makedirs failed for '{name}': {type(e).__name__} - {e}")
                raise # Re-raise the original error
        else:
            logger.error(f"Makedirs: Path '{abs_target_path}' is NOT whitelisted for creation. Denying operation.")
            raise PermissionError(f"File system makedirs operation to '{abs_target_path}' is not allowed.")

    def restricted_mkdir(self, path, mode=0o777, *, dir_fd=None):
        logger.debug(f"restricted_mkdir called for path: '{path}', mode: {oct(mode)}, dir_fd: {dir_fd}")
        ...
        if dir_fd is not None:
            logger.warning(f"restricted_mkdir with dir_fd: Whitelist check for path '{path}' will use os.path.abspath, which may not correctly resolve relative paths against dir_fd. This scenario is not fully secured by the current whitelist logic if 'path' is relative.")
        ...
        if self.is_directory_whitelisted(abs_target_path):
            logger.info(f"Mkdir: Path '{abs_target_path}' is whitelisted. Calling original os.mkdir.")
            try:
                # Pass dir_fd explicitly as it's a keyword-only argument
                return self.original_mkdir(path, mode, dir_fd=dir_fd)
            except Exception as e:
                logger.error(f"Original os.mkdir failed for '{path}': {type(e).__name__} - {e}")
                raise # Re-raise the original error
        else:
            logger.error(f"Mkdir: Path '{abs_target_path}' is NOT whitelisted. Denying operation.")
            raise PermissionError(f"File system mkdir operation to '{abs_target_path}' is not allowed.")
    ...
    def restricted_remove(self, path, *, dir_fd=None):
        ...
        if dir_fd is not None:
            logger.warning(f"restricted_remove with dir_fd: Whitelist check for path '{path}' will use os.path.abspath. This scenario is not fully secured if 'path' is relative.")
        ...
        if self.is_path_whitelisted(abs_target_path):
            logger.info(f"Remove: Path '{abs_target_path}' is whitelisted for removal. Calling original os.remove.")
            try:
                return self.original_remove(path, dir_fd=dir_fd)
            except Exception as e:
                logger.error(f"Original os.remove failed for '{path}': {type(e).__name__} - {e}")
                raise
        else:
            logger.error(f"Remove: Path '{abs_target_path}' is NOT whitelisted for removal. Denying operation.")
            raise PermissionError(f"File system remove operation on '{abs_target_path}' is not allowed.")

    def restricted_rmdir(self, path, *, dir_fd=None):
        logger.debug(f"restricted_rmdir called for path: '{path}', dir_fd: {dir_fd}")
        if dir_fd is not None:
            logger.warning(f"restricted_rmdir with dir_fd: Whitelist check for path '{path}' will use os.path.abspath. This scenario is not fully secured if 'path' is relative.")
        ...
        if self.is_directory_whitelisted(abs_target_path):
            logger.info(f"Rmdir: Path '{abs_target_path}' is whitelisted for removal. Calling original os.rmdir.")
            try:
                return self.original_rmdir(path, dir_fd=dir_fd)
            except Exception as e:
                logger.error(f"Original os.rmdir failed for '{path}': {type(e).__name__} - {e}")
                raise
        else:
            logger.error(f"Rmdir: Path '{abs_target_path}' is NOT whitelisted for removal. Denying operation.")
            raise PermissionError(f"File system rmdir operation on '{abs_target_path}' is not allowed.")
