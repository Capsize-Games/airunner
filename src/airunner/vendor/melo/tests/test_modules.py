...
def test_Log_forward_and_reverse():
    log = modules.Log()
    x = torch.abs(torch.randn(2, 4, 5)) + 1e-2
    x_mask = torch.ones(2, 1, 5)
...
def test_TransformerCouplingLayer_forward_and_reverse():
    tcl = modules.TransformerCouplingLayer(4, 4, 3, 3, 2, filter_channels=4)
    x = torch.randn(2, 4, 5)
    x_mask = torch.ones(2, 1, 5)
    y, logdet = tcl(x, x_mask)
    assert y.shape == x.shape
    x_recon = tcl(y, x_mask, reverse=True)
    assert x_recon.shape == x.shape
