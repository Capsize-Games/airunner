from airunner.FramePack.diffusers_helper.hf_login import login
...
from airunner.FramePack.diffusers_helper.hunyuan import (
    encode_prompt_conds,
    vae_decode,
    vae_encode,
    vae_decode_fake,
)
from airunner.FramePack.diffusers_helper.utils import (
    save_bcthw_as_mp4,
...
from airunner.FramePack.diffusers_helper.models.hunyuan_video_packed import (
    HunyuanVideoTransformer3DModelPacked,
)
from airunner.FramePack.diffusers_helper.pipelines.k_diffusion_hunyuan import (
    sample_hunyuan,
)
from airunner.FramePack.diffusers_helper.memory import (
    cpu,
    gpu,
...
from airunner.FramePack.diffusers_helper.thread_utils import (
    AsyncStream,
    async_run,
)
from airunner.FramePack.diffusers_helper.gradio.progress_bar import (
    make_progress_bar_css,
    make_progress_bar_html,
)
from transformers import SiglipImageProcessor, SiglipVisionModel
from airunner.FramePack.diffusers_helper.clip_vision import (
    hf_clip_vision_encode,
)
from airunner.FramePack.diffusers_helper.bucket_tools import (
    find_nearest_bucket,
)
...
def worker(
    ...
    total_latent_sections = (total_second_length * 30) / (
    ...
    try:
        ...
        stream.output_queue.push(
            (
                "progress",
                (None, "", make_progress_bar_html(0, "Text encoding ...")),
            )
        )
        ...
        stream.output_queue.push(
            (
                "progress",
                (None, "", make_progress_bar_html(0, "VAE encoding ...")),
            )
        )
        ...
        for latent_padding in latent_paddings:
            ...
            save_bcthw_as_mp4(
                history_pixels, output_filename, fps=30, crf=mp4_crf
            )
...
def process(
    ...
    yield None, None, "", "", gr.update(interactive=False), gr.update(
        interactive=True
    )
    ...
    while True:
        ...
        if flag == "file":
            ...
            yield output_filename, gr.update(), gr.update(), gr.update(), gr.update(
                interactive=False
            ), gr.update(
                interactive=True
            )
        ...
        if flag == "progress":
            ...
            ), desc, html, gr.update(interactive=False), gr.update(
                interactive=True
            )

        if flag == "end":
            yield output_filename, gr.update(
                visible=False
            ), gr.update(), "", gr.update(interactive=True), gr.update(
                interactive=False
            )
            break
        ...
        with gr.Column():
            ...
            with gr.Row():
                ...
                end_button = gr.Button(
                    value="End Generation", interactive=False
                )
