    ...
    def do_generate_image(self):
        ...
        if image is not None:
            ...
            if overlap_left < overlap_right and overlap_top < overlap_bottom:
                print("WE HAVE OVERLAP", overlap_left, overlap_top, overlap_right, overlap_bottom)
                print(active_rect.left(), active_rect.top(), active_rect.right(), active_rect.bottom())
                print(self.settings["working_width"], self.settings["working_height"])
                draw.rectangle((overlap_left, overlap_top, overlap_right, overlap_bottom), fill='white')

                # Crop the image at the overlap position
                cropped_image = image.crop((image_overlap_left, image_overlap_top, image_overlap_right, image_overlap_bottom))
                ...
                # Emit the signal with the new image and the mask
                self.emit_signal(SignalCode.DO_GENERATE_SIGNAL, {
                    "mask_image": mask.convert("RGB"),
                    "image": new_image.convert("RGB")
                })
            else:
                base64mask = convert_image_to_base64(mask)
                settings = self.settings
                settings["canvas_settings"]["mask"] = base64mask
                self.settings = settings
