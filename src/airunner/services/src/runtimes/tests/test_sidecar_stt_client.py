...
def test_register_sidecar_stt_client_overrides_default_stt_route():
    registry = RuntimeRegistry()
    fallback = object()
    client = SidecarSTTClient(
        settings=_settings(),
    ...
    registry.register(RuntimeRoute(RuntimeKind.STT, provider="local"), fallback)
    registry.register(
        RuntimeRoute(
            RuntimeKind.STT,
            provider="local",
    ...
    assert registry.resolve(RuntimeKind.STT, provider="local") is client
