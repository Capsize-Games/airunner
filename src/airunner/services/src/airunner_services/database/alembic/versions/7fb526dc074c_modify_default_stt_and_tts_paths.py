...
from airunner_services.utils.db import safe_alter_column
...
def downgrade() -> None:
    stt_model_path = os.path.expanduser(
        os.path.join(AIRUNNER_BASE_PATH, "text", "models", "stt", "models")
    )
    tts_model_path = os.path.expanduser(
        os.path.join(AIRUNNER_BASE_PATH, "text", "models", "tts", "models")
    )
    safe_alter_column(
        PathSettings,
        "stt_model_path",
...
def update_path_values(stt_model_path, tts_model_path):
    ...
    for path_setting in path_settings:
        ...
        stt_model_path = stt_model_path.replace(
            os.path.expanduser(AIRUNNER_BASE_PATH), base_path
        )
        tts_model_path = tts_model_path.replace(
            os.path.expanduser(AIRUNNER_BASE_PATH), base_path
        )
        ...
        try:
            connection.execute(
                text(
                    "UPDATE path_settings SET stt_model_path = :stt, tts_model_path = :tts WHERE id = :id"
                ),
