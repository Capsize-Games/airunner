"""Tests for the service-owned runtime layout helpers."""

from __future__ import annotations

from airunner_native.linux_bundle_layout import build_linux_bundle_layout
from airunner_services.config.runtime_layout import build_runtime_directory_layout
from airunner_services.config.runtime_layout import resolve_runtime_bind_host
from airunner_services.daemon_config import DaemonConfig
from airunner_services.service_manager import LinuxSystemdHandler
...
def test_linux_systemd_service_content_includes_runtime_boundary(
    ...
    content = handler._generate_service_content(
        "python -m airunner_services.daemon",
        config_path,
    )
    ...
    assert (
        f"ExecStart=python -m airunner_services.daemon --config {config_path}"
        in content
    )
