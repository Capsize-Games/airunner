...
class NodeGraphWidget(BaseWidget):
    ...
    def __init__(self, parent=None, *args, **kwargs):
        ...
        self.node_graph_worker, self._node_graph_thread = create_worker(NodeGraphWorker)
        self._register_nodes()
        self._initialize_context_menu()
        self._register_graph()

        self.framepack_worker, self._framepack_thread = (None, None)
        here = os.path.dirname(__file__)
        if os.path.exists(os.path.join(here, "../../FramePack")):
            from airunner.workers.framepack_worker import FramePackWorker

            self.framepack_worker, self._framepack_thread = create_worker(
                FramePackWorker
            )
        ...
        if os.path.exists(os.path.join(here, "../../FramePack")):
            ...
            self.framepack_worker, _ = create_worker(FramePackWorker)
    ...
    def closeEvent(self, event):
        ...
        for worker, thread in [
            (
                getattr(self, "node_graph_worker", None),
                getattr(self, "_node_graph_thread", None),
            ),
            (
                getattr(self, "framepack_worker", None),
                getattr(self, "_framepack_thread", None),
            ),
        ]:
            if worker:
