...
class PygameWindow(
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if AIRUNNER_SD_ON:
            self._mask_generator_worker, _ = create_worker(MaskGeneratorWorker)
            self._sd_worker, _ = create_worker(SDWorker)

        if AIRUNNER_STT_ON:
            self._stt_audio_capture_worker, _ = create_worker(AudioCaptureWorker)
            self._stt_audio_processor_worker, _ = create_worker(AudioProcessorWorker)

        if AIRUNNER_TTS_ON:
            self._tts_generator_worker, _ = create_worker(TTSGeneratorWorker)
            self._tts_vocalizer_worker, _ = create_worker(TTSVocalizerWorker)
    ...
    def closeEvent(self, event):
        # Ensure all worker threads are stopped and joined on close
        for attr in [
            "_mask_generator_worker",
            "_sd_worker",
            "_stt_audio_capture_worker",
            "_stt_audio_processor_worker",
            "_tts_generator_worker",
            "_tts_vocalizer_worker",
