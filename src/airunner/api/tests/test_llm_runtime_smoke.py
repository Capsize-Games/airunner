...
from airunner.ipc.messages import EnvelopeStatus, ResponseEnvelope, StreamDelta
...
def test_llm_runtime_load_and_unload_use_control_actions(monkeypatch):
    ...
    from airunner.api.routes.llm import ModelLoadRequest, load_model, unload_model
    ...
    load_response = asyncio.run(
        load_model(ModelLoadRequest(model_id="qwen3-8b"), req)
    )
    unload_response = asyncio.run(unload_model(req))
...
def test_llm_websocket_stream_relays_runtime_deltas():
    ...
    from airunner.api.routes.llm import websocket_chat
