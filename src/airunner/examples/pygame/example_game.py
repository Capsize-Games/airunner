...
class ExampleGame(PygameAdapter):
    ...
    def handle_pygame_event(self, event):
        ...
        if event.type == QUIT:
            ...
            if event.key == K_ESCAPE:
                ...
                self.full_message = ""
                self.api.llm.send_request("Tell me a joke.", do_tts_reply=True)
    ...
    def update(self):
        ...
        self.player_pos[0] = max(
            self.player_size // 2,
            min(self.width - self.player_size // 2, self.player_pos[0]),
        )
        self.player_pos[1] = max(
            self.player_size // 2,
            min(self.height - self.player_size // 2, self.player_pos[1]),
        )
    ...
    def render(self):
        ...
        current_time = time.time()
        if (
            hasattr(self, "show_message")
            and hasattr(self, "show_message_time")
            and current_time < self.show_message_time
        ):
