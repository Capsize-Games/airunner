"""Helpers for validating user-controlled local filesystem paths."""
...
class PathPolicyError(ValueError):
    """Raised when one path violates AIRunner's local path policy."""
...
def _normalized_suffixes(
    ...
    """Return one normalized suffix set."""
...
def _validate_suffix(
    ...
    *,
    ...
    """Validate one file suffix against an allow list."""
...
def _path_within_root(candidate: Path, root: Path) -> bool:
    """Return True when one candidate path lives inside one root."""
    try:
...
def _validate_roots(
    ...
    *,
    ...
    """Validate that one candidate lives inside an allowed root."""
