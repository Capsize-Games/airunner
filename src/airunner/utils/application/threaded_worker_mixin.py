...
class ThreadedWorkerMixin:
    ...
    def execute_in_background(
        ...
        worker = BackgroundWorker(
            task_function=task_function, callback_data=callback_data or {}
        )
        ...
        if on_finished:
            ...
            if hasattr(worker, "_signal_refs"):
                worker._signal_refs.append(finished_lambda)
        else:
            ...
            if hasattr(worker, "_signal_refs"):
                worker._signal_refs.append(cleanup_lambda)

        # DEBUG: print all connected callbacks (only for DummySignal)
        if hasattr(worker.taskFinished, "_callbacks"):
            print(
                f"ThreadedWorkerMixin: taskFinished callbacks for {task_id}: {worker.taskFinished._callbacks}"
            )
        else:
            print(
                f"ThreadedWorkerMixin: taskFinished is not DummySignal for {task_id}, type: {type(worker.taskFinished)}"
            )
    ...
    def _cleanup_worker(self, task_id: str) -> None:
        ...
        print(f"ThreadedWorkerMixin: _cleanup_worker called for {task_id}")
