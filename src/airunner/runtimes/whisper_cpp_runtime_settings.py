...
def _discover_model_path(model_directory: str) -> Optional[str]:
    ...
    fallback = sorted(candidate.rglob("*.bin"))
    if fallback:
...
def resolve_whisper_cpp_runtime_settings() -> WhisperCppRuntimeSettings:
    """Resolve whisper.cpp settings from environment and persisted state."""
    path_settings = _load_path_settings()
    base_path = getattr(path_settings, "base_path", AIRUNNER_BASE_PATH)
    model_path = _candidate_model_path(base_path, path_settings)
