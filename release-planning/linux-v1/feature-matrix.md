...
| ART-04 | Image filters (blur, dither, halftone, pixel art, color balance, etc.) | `src/airunner/components/art/filters/*.py` (11 filter implementations); UI: `gui/widgets/filter_parameter_widget.py`, `gui/windows/filter_window/filter_window.py` | local | manual | No automated filter-specific test found, but [Q02](https://github.com/Capsize-Games/airunner/issues/2122)'s outcome explicitly names "filter" among the scenarios to run ("Run every inventoried art/canvas mode, LoRA/embeddings, batch, filter, import/export and editing scenario"). |
...
| ART-06 | VRAM-aware generation settings / budget calculator | `src/airunner/components/art/gui/widgets/stablediffusion/stable_diffusion_settings_widget.py`, `vram_calculator.py` | gpu | gap | No test covers `vram_calculator.py` itself; `test_loading_watchdog.py` (same directory) only covers the loading watchdog (`test_watchdog_releases_pinned_ui_at_startup`, `test_real_backend_progress_clears_watchdog`, etc.), not VRAM budgeting. Proposed: FOLLOWUP-1. |
...
| ART-13 | Embedding (textual inversion) management: scanning, loading, applying to a generation | `src/airunner/components/art/gui/widgets/embeddings/embeddings_container_widget.py` (`scan_for_embeddings`, `load_embeddings`, `apply_embeddings`), `embedding_trigger_word_widget.py`, `embedding_widget.py`; `src/airunner/components/art/api/embedding_services.py`, `utils/embeddings.py` | gpu | gap | No test found. This is a distinct SD textual-inversion embedding feature, not to be confused with RAG-04/RAG-06 (LLM/vector-search embeddings). Proposed: FOLLOWUP-1. |
...
| RAG-05 | RAG indexing / search / tool integration | `services/src/airunner_services/llm/managers/agent/mixins/rag_indexing_mixin.py`, `rag_document_mixin.py`, `rag_search_mixin.py`; `services/src/airunner_services/llm/tools/rag_tools.py` | local | auto | `services/tests/eval/test_agent_document_eval.py` |
...
1. **FOLLOWUP-1 (recommend before release): triage untested network/security-sensitive tool and data-source paths.** Covers ART-06/ART-07/ART-13 (VRAM budget calculator, RMBG, embedding management — no test, but low risk; ART-04's filters are already named as an acceptance scenario by [Q02](https://github.com/Capsize-Games/airunner/issues/2122), so it is excluded here), RAG-02 (Kiwix), SRV-07 (geolocation — confirm it's even reachable), TOOL-03/04/05 (web search, arXiv, crawler), and especially **TOOL-06 (local filesystem access tool)**, which has no dedicated regression test despite being a direct arbitrary-local-file-access surface exposed to an LLM. Recommend scoping TOOL-06 as its own higher-priority ticket rather than bundling it with the lower-risk items above.
