...
- https://github.com/Capsize-Games/airunner/issues/2137
- https://github.com/Capsize-Games/airunner/issues/2134
- https://github.com/Capsize-Games/airunner/issues/2091

## Read/change boundary

- `services/src/airunner_services/llm/companion/`
- `services/src/airunner_services/runtimes/registry.py`
...
- [ ] Fake routing matrix covers every pipeline from W01.
- [ ] Offline has zero external inference/embedding requests.
- [ ] Cancellation and exhausted VRAM/resource admission yield actionable results instead of loading duplicate models.

## Validation

Add the focused regression file `services/tests/test_release_b12.py` (or extend the named existing suite and report its exact equivalent command). Use real data contracts with fake external/runtime boundaries, no actual model/network/database side effects.

```bash
AIRUNNER_TEST_NO_GUI_LAUNCH=1 AIRUNNER_HEADLESS=1 venv/bin/python -m pytest services/tests/test_release_b12.py -o addopts='' -q
```
