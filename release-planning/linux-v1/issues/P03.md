...
- `.github/workflows/native-runtime-sidecars.yml`
...
- [ ] Bundle manifest records exact upstream commits, target architecture and CUDA options.
- [ ] CI artifact contents match paths used by runtime discovery.
- [ ] No reliance on developer absolute paths or undocumented system binaries.

## Validation

Add the focused regression file `services/tests/test_release_p03.py` (or extend the named existing suite and report its exact equivalent command). Use real data contracts with fake external/runtime boundaries, no actual model/network/database side effects.

```bash
AIRUNNER_TEST_NO_GUI_LAUNCH=1 AIRUNNER_HEADLESS=1 venv/bin/python -m pytest services/tests/test_release_p03.py -o addopts='' -q
```
