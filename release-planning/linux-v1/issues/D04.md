...
- [ ] Negative, zero, excessive, nonfinite and malformed values are rejected early.
- [ ] Valid boundary cases pass and downstream runtime errors remain separate.
- [ ] No GPU/network used.

## Validation

Add the focused regression file `services/tests/test_release_d04.py` (or extend the named existing suite and report its exact equivalent command). Use real data contracts with fake external/runtime boundaries, no actual model/network/database side effects.

```bash
AIRUNNER_TEST_NO_GUI_LAUNCH=1 AIRUNNER_HEADLESS=1 venv/bin/python -m pytest services/tests/test_release_d04.py -o addopts='' -q
```
