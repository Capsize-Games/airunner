...
- `services/src/airunner_services/llm/managers/mixins/system_prompt_mood.py`
...
- [ ] Neutral fixtures match intended section ordering and optional omissions.
- [ ] Changing one chatbot cannot affect another prompt.
- [ ] Oversized memory truncates by a documented policy without dropping mandatory safety instructions.

## Validation

Add the focused regression file `services/tests/test_release_b10.py` (or extend the named existing suite and report its exact equivalent command). Use real data contracts with fake external/runtime boundaries, no actual model/network/database side effects.

```bash
AIRUNNER_TEST_NO_GUI_LAUNCH=1 AIRUNNER_HEADLESS=1 venv/bin/python -m pytest services/tests/test_release_b10.py -o addopts='' -q
```
