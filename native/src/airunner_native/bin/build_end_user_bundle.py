...
from airunner_native.distribution.bundle_layout import bundle_archive_name
from airunner_native.distribution.bundle_layout import file_sha256
from airunner_native.distribution.bundle_layout import relative_manifest_path
...
class BundleBuilder:
    ...
    def install_airunner(self, python_executable: Path) -> None:
        ...
        project_specs = build_project_specs(
...
def read_airunner_version(repo_root: Path) -> str:
    """Read the AIRunner version from one canonical package metadata file."""
    module_path = repo_root / "native" / "package_metadata.py"
    spec = importlib.util.spec_from_file_location(
        "airunner_native_package_metadata",
        module_path,
    )
    if spec is None or spec.loader is None:
        raise RuntimeError(f"Unable to load version metadata: {module_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module.VERSION


def build_project_specs(repo_root: Path, spec: BundleSpec) -> list[str]:
    """Return the local package specs installed into one staged bundle."""
    service_spec = str(repo_root / "services")
    if spec.install_extra:
        service_spec = f"{service_spec}[{spec.install_extra}]"
    return [
        str(repo_root / "model"),
        str(repo_root / "api"),
        service_spec,
