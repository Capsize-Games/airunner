...
_HEADLESS_VALUES = {"1", "true", "yes", "on"}
...
def _facehugger_activate_kwargs() -> dict[str, object]:
    """Return Facehugger activation settings for the desktop process."""
    airunner_path = AIRUNNER_BASE_PATH
    ...
    return {
        ...
        "darklock_os_allow_network": True,
        ...
        "darklock_os_whitelisted_directories": [
            airunner_path,
            os.path.join(airunner_path, "cache"),
            os.path.join(airunner_path, "cache/.webcache"),
            os.path.join(airunner_path, "code"),
            os.path.join(airunner_path, "data"),
            os.path.join(airunner_path, "static"),
            ...
            os.path.join(os.path.expanduser("~"), ".cache/airunner-rag/"),
...
def _find_gui_package_dir() -> Optional[Path]:
    """Return the installed GUI package directory when available."""
    spec = importlib.util.find_spec("airunner")
    if spec is None or spec.origin is None:
        return None
    return Path(spec.origin).resolve().parent


def _require_gui_package_dir() -> Path:
    """Return the GUI package directory or raise one actionable error."""
    package_dir = _find_gui_package_dir()
    if package_dir is not None:
        return package_dir
    raise RuntimeError(
        "The AIRunner GUI package is not installed. Install "
        '"airunner-native[gui]" or install the "airunner" package '
        "before launching the desktop app."
    )


def _components_path() -> Path:
    """Return the installed GUI components directory."""
    return _require_gui_package_dir() / "components"


def _parse_launcher_args(
    argv: Optional[Sequence[str]] = None,
) -> tuple[bool, list[str]]:
    """Return whether headless mode is requested and sanitized args."""
    requested = os.environ.get("AIRUNNER_HEADLESS", "0")
    headless_requested = requested.strip().lower() in _HEADLESS_VALUES
    ...
    return headless_requested, sanitized_args


def _run_headless_launcher(
    argv: Optional[Sequence[str]] = None,
) -> int:
    """Delegate execution to the service-owned headless entry point."""
    from airunner_services.bin.airunner_headless import main as headless_main
...
def build_ui_if_needed():
    ...
    ui_build_marker = _components_path() / "ui_build_marker"
...
def _show_early_splash(existing_app=None):
    ...
    image_path = (
        _require_gui_package_dir() / "gui" / "images" / "splashscreen.png"
    )
...
def _run_desktop_launcher(startup_started_at: float) -> int:
    """Execute the desktop bootstrap flow."""
    _require_gui_package_dir()
    _assert_test_gui_launch_allowed()
    _activate_facehugger_shield()
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    configure_noisy_loggers()
    headless_requested, sanitized_args = _parse_launcher_args(argv)
    if headless_requested:
        return _run_headless_launcher(sanitized_args)
    if argv is not None:
        sys.argv = [sys.argv[0], *sanitized_args]
    return _run_desktop_launcher(startup_started_at)
