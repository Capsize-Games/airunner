"""Service-owned runtime extension loader."""
...
from airunner_services.extension_manifest import (
...
def _iter_extension_packages(extension_root: Path) -> Iterable[Path]:
    ...
    children = [path for path in extension_root.iterdir() if path.is_dir()]
...
def _ensure_parent_package() -> None:
    """Create the stable parent namespace used for loaded extensions."""
...
def load_extensions(force_reload: bool = False) -> dict:
    """Load extensions from configured `extensions/` folders."""
