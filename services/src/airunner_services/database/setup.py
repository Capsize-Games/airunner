...
from airunner_services.utils.db.engine import create_configured_engine
...
def _database_assets_root() -> Path:
    """Return the service-owned database assets root."""
    return Path(__file__).resolve().parent


def _repo_root(start: Path) -> Path | None:
    """Return the AIRunner repository root when running from a checkout."""
...
def _version_locations(base: Path, alembic_dir: Path) -> list[Path]:
    ...
    for extensions_root in (
        repo_root / "gui" / "src" / "extensions",
        repo_root / "src" / "extensions",
    ):
...
def _repair_application_schema(target_db_url: str) -> None:
    ...
    import airunner_services.application_data as application_data
...
def setup_database(db_url: str | None = None):
    ...
    assets_root = _database_assets_root()
    repo_root = _repo_root(assets_root) or assets_root
    alembic_file = assets_root / "alembic.ini"
    alembic_dir = assets_root / "alembic"
    with _SETUP_LOCK:
        if _use_setup_cache() and target_db_url in _COMPLETED_SETUP_URLS:
            return
        ...
        version_locations = _version_locations(assets_root, alembic_dir)
