...
def load_model(adapter: Any) -> None:
    ...
    _configure_chat_handler(adapter)
...
def _configure_chat_handler(adapter: Any) -> None:
    """Install one HF chat-template handler when the model needs it."""
    if getattr(adapter, "_llama", None) is None:
        return
    repo_id = _chat_template_repo_id(adapter)
    if not repo_id:
        return
    base_handler = _build_hf_chat_handler(adapter, repo_id)
    ...
    adapter.logger.info(
        "Using HF chat template handler for %s",
        repo_id,
    )


def _chat_template_repo_id(adapter: Any) -> str | None:
...
def _load_chat_template_tokenizer(adapter: Any, repo_id: str) -> Any | None:
