"""Service-owned HuggingFace download helpers with progress hooks."""
...
class DownloadCancelledError(RuntimeError):
    """Raised when one provider download is cancelled."""
...
class HuggingFaceDownloader:
    ...
    def _get_huggingface_api_key(self) -> str:
        """Return one persisted HuggingFace token when available."""
        return get_setting("huggingface/api_key", "")
                ...
                for chunk in response.iter_content(chunk_size=8192):
                    if cancel_callback and cancel_callback():
                        ...
                        raise DownloadCancelledError(
                            f"Cancelled download for {filename}"
                        )
                    if chunk:
                        f.write(chunk)
