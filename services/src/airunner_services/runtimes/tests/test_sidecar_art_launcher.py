...
from airunner_services.runtimes import (
    sidecar_art_launcher as sidecar_art_launcher_module,
)
...
def test_start_spawns_daemon_with_art_only_environment(
    tmp_path,
    monkeypatch,
):
    process = FakeProcess()
    ...
    launcher = SidecarArtLauncher(
        ...
        bundle_layout_builder=lambda: bundle_layout,
    )

    launcher.start()

    assert Path(captured["command"][0]).name == "python"
    ...
    assert environment["PATH"].startswith(
        str(bundle_layout.python_executable.parent)
    )
...
def test_prepare_managed_daemon_launch_skips_launcher_import_outside_tests(
    ...
    def fake_import(name, globals=None, locals=None, fromlist=(), level=0):
        ...
        if name == "airunner_native.launcher":
