...
class LLMGenerateWorker(
	...
	def _check_inactivity(self) -> None:
		...
		if self._model_manager.model_status.get(ModelType.LLM) != ModelStatus.LOADED:
			return
	...
	def run(self) -> None:
		...
		which no production code calls) is what makes the LAN-daemon
	...
	def handle_message(self, message: Dict) -> None:
		...
		if llm_request.rag_files is not None and len(llm_request.rag_files) > 0:
			self.logger.info(
