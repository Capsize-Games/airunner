...
class KnowledgeBase:
	...
	def add_fact(
		...
		path = self._ensure_today_file() if date_str is None else self._get_file_path(
			date_str
		)
	...
	def update_fact(
		...
		files = [self._get_file_path(date_str)] if date_str else self.list_files()
		total_count = 0
	...
	def search(self, query: str, max_results: int = 10) -> List[Dict[str, str]]:
		...
		min_score = max(2, len(query_words) // 2) if len(query_words) > 2 else 1
		results = []
		...
		for path in self.list_files():
			...
			for index, line in enumerate(lines):
				...
				if score < min_score or not line.strip() or line.startswith("#"):
					continue
	...
	def _register_knowledge_documents(self, files: List[str]) -> None:
		try:
			...
			self.logger.error("Failed to register knowledge documents: %s", exc)
