"""
Math problem-solving benchmark harness: self-verification loops with
retry logic and math-specific prompting.

The Python-execution sandbox this module used to define
(SafePythonExecutor and its session helpers) is production code, not
benchmark tooling -- it backs the sympy_compute/numpy_compute/
python_compute LLM tools -- and moved to
airunner_services.llm.core.math_executor (issue #2189). Import it
from there; it is re-imported here only because
SelfVerificationSolver still uses it.
"""
...
from airunner_services.llm.core.math_executor import SafePythonExecutor
