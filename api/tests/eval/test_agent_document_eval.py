...
def _document_daemon_env() -> dict[str, str]:
...
def _document_request(prompt: str) -> dict[str, object]:
...
def test_agent_routes_codename_document_result_through_rag_search() -> None:
    ...
    with started_daemon(_document_daemon_env()) as daemon:
        result = run_agent_eval_case(
            daemon.base_url,
            _document_request(prompt),
        )
...
@pytest.mark.slow
@pytest.mark.timeout(900)
def test_agent_routes_numeric_document_result_through_rag_search() -> None:
