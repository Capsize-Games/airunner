...
def cleanup_stale_download_dir(
    ...
    try:
        ...
        if logger:
            logger.warning("Failed to remove stale download temp dir %s: %s", path, exc)
        return False
...
def cleanup_stale_download_dirs(
    ...
    for temp_dir in root_path.rglob(".downloading"):
        if cleanup_stale_download_dir(temp_dir, max_age_seconds=max_age_seconds, logger=logger):
