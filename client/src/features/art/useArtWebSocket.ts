...
function wsUrl(): string {
  ...
  return `${proto}://${wsHost()}/api/v1/art/ws`;
}
...
export interface ArtGenerateParams {
  ...
  imageB64?: string;
  /** Mask image for inpaint, raw base64 (no data: prefix). */
  maskB64?: string;
  /** Denoise strength for img2img, 0–1. */
  strength?: number;
    ...
    (params: ArtGenerateParams): Promise<string> => {
      ...
      return new Promise((resolve, reject) => {
        ...
        sendMessage({
          ...
          ...(params.maskB64 ? { mask_b64: params.maskB64 } : {}),
