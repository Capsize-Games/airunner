...
def get_all_available_models() -> Dict[str, List[Dict]]:
    ...
    try:
        ...
        for model_key, info in LLMProviderConfig.LOCAL_MODELS.items():
            ...
            if (
                preferred_download
                and preferred_download.get("model_type") == "gguf"
            ):
                default_format = "GGUF"
                default_runtime = "llama.cpp"
                preferred_repo_id = preferred_download.get(
                    "repo_id", preferred_repo_id
                )
            models["llm"].append(
                {
                    "model_id": model_key,
...
def find_model(
    identifier: str, models: Dict[str, List[Dict]]
) -> Optional[Dict]:
    ...
    for model_type, model_list in models.items():
        for model in model_list:
            ...
            if any(
                _normalize_model_identifier(candidate) == normalized_identifier
...
def print_model_list(
    models: Dict[str, List[Dict]], model_type: Optional[str] = None
):
    ...
    print("=" * 80)
    print(
        f"{dim}Use 'airunner-hf-download <model>' to download (GGUF by default){reset}"
    )
    print(
        f"{dim}Use 'airunner-hf-download --full <model>' for full safetensors{reset}\n"
    )

    types_to_show = (
        [model_type]
        if model_type
        else ["llm", "art", "tts", "stt", "embedding"]
    )
...
def download_model(
    ...
    if model_type == "llm":
        ...
        if output_dir is not None:
            requested_output_dir = os.path.join(output_dir, model_info["name"])
        ...
        if request.model_type == "gguf" and gguf_filename:
    ...
    if output_dir is None:
        ...
        if model_type == "llm":
            ...
            output_dir = os.path.join(
                MODELS_DIR, "art/models", model_info["name"]
            )
        elif model_type == "tts":
            output_dir = os.path.join(
                MODELS_DIR, "text/models/tts", model_info["name"]
            )
        elif model_type == "stt":
            output_dir = os.path.join(
                MODELS_DIR, "text/models/stt", model_info["name"]
            )
    ...
    job_id = asyncio.run(
        job_service.start_huggingface_download(
            repo_id=repo_id,
            model_type="gguf" if gguf_filename else model_type,
            output_dir=model_output_dir,
            gguf_filename=gguf_filename,
            prefer_pre_quantized=use_gguf,
        )
    )
    return _wait_for_download_job(job_service, job_id, model_name)
...
def _wait_for_download_job(
    ...
    while True:
        ...
        if job is None:
            print(f"\n❌ Error: Download job {job_id} was not found")
            return False

        progress = int(job.progress)
        if progress != last_progress:
            print(f"\r⬇️  {label}: {progress:3d}%", end="", flush=True)
        ...
        if job.status.value == "failed":
            print(f"\n❌ Error downloading {label}: {job.error}")
            return False
        if job.status.value == "cancelled":
            print(f"\n⚠️  Download cancelled: {label}")
            return False
                ...
                downloaded["art"].append(
                    {
                        ...
                        "format": "Diffusion",
                        "files": sum(
                            len(files) for _, _, files in os.walk(model_path)
                        ),
                    }
                )
...
def print_downloaded_models(
    downloaded: Dict[str, List[Dict]], model_type: Optional[str] = None
):
    ...
    print("=" * 80)
    print(
        f"{dim}Use 'airunner-hf-download --delete <model>' to remove a model{reset}\n"
    )
    ...
    for mtype in types_to_show:
        ...
        for model in downloaded[mtype]:
            ...
            print(
                f"    {dim}Size: {size_str} | Format: {model['format']} | Files: {model['files']}{reset}"
            )
    ...
    if total_models == 0:
        ...
        print(
            f"  {dim}Use 'airunner-hf-download <model>' to download one.{reset}\n"
        )
    else:
        print(
            f"{bold}Total: {total_models} model(s), {total_size:.1f}GB{reset}\n"
        )
...
def main():
    ...
    if model_info:
        ...
        success = download_model(
            model_info, use_gguf=use_gguf, output_dir=args.output
        )
        return 0 if success else 1
    ...
    for mtype, mlist in models.items():
        if mlist:
            ...
            print(
                f"  {mtype}: {', '.join(keys)}{'...' if len(mlist) > 5 else ''}"
            )
