"""Service-owned image export worker."""

import datetime
import os
from typing import Any, Dict, List, Optional

from airunner_services.art.managers.stablediffusion import image_generation
...
SignalCode = signal_code_proxy({"IMAGE_EXPORTED": "image_exported_signal"})


class ImageExportWorker(Worker):
	"""Export generated images and metadata to disk."""
	...
	def _initialize_metadata(
		...
		if metadata_settings.export_metadata:
			...
			if data.get("is_txt2img", False):
				...
				metadata_dict.update({"action": "inpaint"})
