...
from ._bootstrap_resolver import (
    get_bootstrap_data_for_model,
    resolve_bootstrap_files,
    resolve_file_list_from_api,
)
from ._file_verifier import (
    check_existing_file,
    remove_file,
    should_skip_transformer_weights,
)
...
class HuggingFaceDownloadWorker(BaseDownloadWorker):
    ...
    def _resolve_art_download_context(
        repo_id, version=None, pipeline_action=None
    ):
        rv = version
    ...
    def _download_model(
        ...
        self.emit_signal(
            SignalCode.UPDATE_DOWNLOAD_LOG,
            {
                "message": f"Downloading {len(files)} files ({total_gb:.2f} GB) in parallel"
            },
        )
        ...
        for fi in files:
            ...
            self.emit_signal(
                SignalCode.UPDATE_DOWNLOAD_LOG,
                {"message": f"Starting download for {fn}..."},
            )
            t = threading.Thread(
    ...
    def _build_file_list(
        self, model_type, repo_id, resolved_version, model_path, missing_files
    ):
        is_bs = model_type in ("art", "stt", "tts_openvoice", "rmbg")
        full_data = get_bootstrap_data_for_model(
            model_type, resolved_version, "txt2img", repo_id, self.logger
        )
        if model_type == "art" and resolved_version == "Z-Image Turbo":
            missing_files = prune_zimage_missing_files(
                model_path, missing_files, self.logger
            )
            if not missing_files:
                full_data = prune_zimage_bootstrap_files(
                    model_path, full_data, self.logger
                )
        if is_bs:
            return self._build_from_bootstrap(
                model_type, full_data, missing_files, model_path, repo_id
            )
        return self._build_from_api(repo_id, model_type, model_path, full_data)

    def _build_from_bootstrap(
        self, model_type, full_data, missing_files, model_path, repo_id
    ):
        bf = resolve_bootstrap_files(
            model_type, full_data, model_path, missing_files, self.logger
        )
        if not bf:
            self.logger.error(
                "No bootstrap data found for %s/%s!", model_type, repo_id
            )
            self.emit_signal(
                self._failed_signal,
                {"error": f"No bootstrap data for {model_type}/{repo_id}"},
            )
            return None
        files = []
        ...
        for fi in all_f:
            ...
            if fp.exists():
                ...
                if sz > 0 and actual < sz:
                    self.logger.warning(
                        "File %s incomplete (%d vs %d), re-downloading",
                        fn,
                        actual,
                        sz,
                    )
