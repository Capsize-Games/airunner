...
def _ws_url(endpoint: str, path: str = "/ws") -> str:
    ...
    """
    host = endpoint.replace("http://", "ws://").replace("https://", "wss://")
    if path.startswith("/"):
        return f"{host}{path}"
    return f"{host}/{path}"
    ...
    *one* sidecar daemon.  It supports two invocation patterns:
    ...
    def __init__(
        ...
        *,
        ...
        self._pending_streams: dict[str, list[ProgressCallback]] = {}
    ...
    async def _reader_loop(self) -> None:
        ...
        try:
            ...
            if not self._closed:
                ...
                self._fail_pending(
                    WebSocketTransportDisconnected(f"Connection closed: {exc}")
                )
        except Exception as exc:
            logger.error("WebSocket reader error: %s", exc)
            self._fail_pending(
                WebSocketTransportDisconnected(f"Reader error: {exc}")
            )
    ...
    async def _dispatch_stream(
        ...
        for cb in callbacks:
            try:
                ...
                logger.exception("Progress callback failed for %s", request_id)
