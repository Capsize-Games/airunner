...
class SidecarArtClient(RuntimeClient):
    ...
    def __init__(
        ...
        *,
        ...
        if launcher is None and resolved_launcher_factory is None:
            resolved_launcher_factory = SidecarArtLauncher
        ...
        if self._managed_launcher:
            self._launcher = resolved_launcher_factory(
    ...
    def _generate_via_websocket(
        ...
        payload = {
            ...
            "metadata": self._build_invocation_metadata(
                invocation, self._settings
            ),
        }
    ...
    def _settings_for_invocation(
        ...
        return replace(
            self._base_settings,
            art_model_path=invocation.model
            or self._base_settings.art_model_path,
            art_model_version=(
                metadata.get("version")
                or self._base_settings.art_model_version
            ),
            art_scheduler=(
                metadata.get("scheduler") or self._base_settings.art_scheduler
            ),
        )
    ...
    def _wait_for_job(
        ...
        return (
            messages.EnvelopeStatus.FAILED,
            {},
...
def register_sidecar_art_client(
    ...
    if art_client is None:
        raise ValueError(
            "register_sidecar_art_client requires a configured client"
        )
    client = art_client
    registry.register(
        RuntimeRoute(
