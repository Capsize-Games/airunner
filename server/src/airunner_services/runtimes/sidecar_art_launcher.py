"""Process supervision for the isolated art runtime."""
...
ConfigPathBuilder = Callable[[ArtDaemonRuntimeSettings], Path]
BundleLayoutBuilder = Callable[[], LinuxBundleLayout]
...
class SidecarArtLauncher:
    """Own the lifecycle of one dedicated art daemon process."""
    ...
    def __init__(
        ...
        *,
        ...
        bundle_layout_builder: BundleLayoutBuilder = build_linux_bundle_layout,
    ) -> None:
        ...
        self._bundle_layout_builder = bundle_layout_builder
    ...
    def api_base_url(self) -> str:
        """Return the art API base URL exposed by the sidecar daemon."""
        return f"{self.endpoint}/api/v1/art"
    ...
    def _wait_until_ready(self) -> None:
        ...
        while self._time_fn() < deadline:
            ...
            if self._process is not None and self._process.poll() is not None:
                code = self._process.poll()
                self._last_error = f"art runtime exited with code {code}"
                self._cleanup_config()
                raise RuntimeError(self._last_error)
            self._sleep(0.1)

        self.stop()
        self._last_error = "Timed out waiting for art runtime to become ready"
        raise RuntimeError(self._last_error)
    ...
    def _environment(self) -> dict[str, str]:
        ...
        if self.settings.art_model_path:
            environment["AIRUNNER_ART_MODEL_PATH"] = (
                self.settings.art_model_path
            )
        if self.settings.art_model_version:
            environment["AIRUNNER_ART_MODEL_VERSION"] = (
    ...
    def _process_working_directory(self) -> Optional[str]:
        ...
        return str(self._bundle_layout_builder().bundle_root)
