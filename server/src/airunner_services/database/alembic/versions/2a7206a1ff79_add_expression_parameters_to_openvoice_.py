...
def upgrade() -> None:
    ...
    op.execute(
        "UPDATE openvoice_settings SET sdp_ratio = 20 WHERE sdp_ratio IS NULL"
    )
    op.execute(
        "UPDATE openvoice_settings SET noise_scale = 60 WHERE noise_scale IS NULL"
    )
    op.execute(
        "UPDATE openvoice_settings SET noise_scale_w = 80 WHERE noise_scale_w IS NULL"
    )
