...
class AIRunnerClient:
    ...
    def reset_memory(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.post(
                f"{self.base_url}/admin/reset_memory",
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            response.raise_for_status()
            response_data = self._load_generate_response(response)
            if response_data is not None:
                return response_data
            return self._generate_via_streaming(request_data)
        except requests.RequestException as e:
            raise AIRunnerClientError(f"Generate request failed: {e}")

    def _load_generate_response(
        self,
        response: requests.Response,
    ) -> Optional[Dict[str, Any]]:
        """Return parsed JSON response when the endpoint is non-streaming."""
        try:
            response_data = self._process_response(response.json())
        except ValueError:
            return None
    ...
    def _generate_via_streaming(
        self,
        request_data: Dict[str, Any],
    ) -> Dict[str, Any]:
        """Collect a streaming generate response into a single payload."""
        streaming_request = dict(request_data)
        ...
        for line in response.iter_lines():
            ...
            try:
                ...
                continue
            message = chunk.get("message") or chunk.get("text", "")
