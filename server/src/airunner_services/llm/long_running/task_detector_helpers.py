        ...
        if match:
            ...
            if count is not None:
                state.detected_items = [
                    f"item_{index + 1}" for index in range(count)
                ]
...
def apply_research_patterns(prompt_lower: str, state: DetectionState) -> None:
    ...
    for pattern in RESEARCH_PATTERNS:
        ...
        if match:
            record_reason(
                state, 0.85, f"Research pattern detected: {match.group(0)}"
            )
...
def apply_coding_patterns(prompt_lower: str, state: DetectionState) -> None:
    ...
    if coding_score >= 2:
        record_reason(
            state, 0.7, f"Multiple implementation keywords ({coding_score})"
        )
    elif coding_score == 1:
...
def coding_keyword_score(prompt_lower: str) -> int:
    ...
    return sum(
        1 for keyword in CODING_PROJECT_KEYWORDS if keyword in prompt_lower
    )
...
def compound_coding_score(prompt_lower: str, state: DetectionState) -> int:
    ...
    for pattern in CODING_COMPOUND_PATTERNS:
        if re.search(pattern, prompt_lower):
            record_reason(
                state, 0.75, f"Compound coding pattern: {pattern[:30]}"
            )
            return 2
    return 0
