...
def start_workflow(
    ...
    if (
        state.workflow_type is not None
        and state.workflow_type != WorkflowType.SIMPLE
    ):
        current_phase = (
...
def get_workflow_status() -> str:
    ...
    completed = sum(
        1 for t in state.todo_list if t.status == TodoStatus.COMPLETED
    )
    in_progress = sum(
        1 for t in state.todo_list if t.status == TodoStatus.IN_PROGRESS
    )
    ...
    if next_todo and next_todo.id != (
        current_todo.id if current_todo else None
    ):
...
@tool(
    ...
    input_examples=[
        ...
        {
            "phase": "execution",
            "reason": "TODO items created, ready to implement",
        },
    ...
    input_examples=[
        {
            ...
            "description": "Create test_hello.py with test for hello_world function",
        },
        {
...
@tool(
    ...
    input_examples=[
        {
            "todo_id": "todo_1",
            "summary": "Created test_hello.py with unit test",
        },
        {
            "todo_id": "todo_2",
            "summary": "Implemented hello_world function",
            "artifacts_json": '{"file": "hello.py"}',
        },
    ],
)
