...
class ZImageImg2ImgPipeline(DiffusionPipeline, FromSingleFileMixin):
    r"""
    ...
    def prepare_latents(
        ...
        t = (
            timestep.item() / 1000.0
            if timestep.dim() == 0
            else timestep[0].item() / 1000.0
        )
    ...
    def __call__(
        ...
        init_image = self.prepare_image(
            image, width, height, device, self.vae.dtype
        )
