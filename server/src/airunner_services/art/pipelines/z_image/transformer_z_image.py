...
# Once ZImageTransformer2DModel is available in a stable diffusers release, this
...
# Try to import from diffusers, fall back to local implementation
try:
...
class ZSingleStreamAttnProcessor:
    ...
    def __call__(
        ...
        # Note: parallel_config is not supported in diffusers 0.35.x
        # Only pass arguments that dispatch_attention_fn accepts
        ...
        with torch.device("cpu"):
            ...
            for i, (d, e) in enumerate(zip(dim, end)):
                freqs = 1.0 / (
                    ...
                    ** (
                        torch.arange(
                            0, d, 2, dtype=torch.float64, device="cpu"
                        )
                        / d
                    )
                )
                timestep = torch.arange(
                    e, device=freqs.device, dtype=torch.float64
                )
