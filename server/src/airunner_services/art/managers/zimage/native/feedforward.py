...
class FeedForward(nn.Module):
    ...
    def __init__(
        ...
        self.w2 = nn.Linear(
            hidden_dim, dim, bias=False, device=device, dtype=dtype
        )
        self.w3 = nn.Linear(
            dim, hidden_dim, bias=False, device=device, dtype=dtype
        )

    def _forward_silu_gating(
        self, x1: torch.Tensor, x3: torch.Tensor
    ) -> torch.Tensor:
        """
...
class MLPBlock(nn.Module):
    ...
    def __init__(
        ...
        self.fc1 = nn.Linear(
            dim, hidden_dim, bias=bias, device=device, dtype=dtype
        )
        self.fc2 = nn.Linear(
            hidden_dim, dim, bias=bias, device=device, dtype=dtype
        )
