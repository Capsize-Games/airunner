...
class TimestepEmbedder(nn.Module):
    ...
    def forward(
        self, t: torch.Tensor, dtype: torch.dtype = None
    ) -> torch.Tensor:
        """
        Forward pass.
...
def _apply_rope_single(
    ...
    ), f"Shape mismatch: {result.shape} vs {original_shape}"
...
class Timestep(nn.Module):
    ...
    def forward(self, t: torch.Tensor) -> torch.Tensor:
        ...
        """
        return TimestepEmbedder.timestep_embedding(t.flatten(), self.dim)
