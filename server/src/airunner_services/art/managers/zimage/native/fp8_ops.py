...
def register_layout_op(
    torch_op: Any, layout_type: str
...
class QuantizedTensor(torch.Tensor):
    ...
    def __tensor_unflatten__(
        inner_tensors: dict, ctx: dict, outer_size: tuple, outer_stride: tuple
    ) -> "QuantizedTensor":
        """
        ...
        return QuantizedTensor(
            inner_tensors["_qdata"], layout_type, layout_params
        )

    @classmethod
    def from_float(
        cls, tensor: torch.Tensor, layout_type: str, **quantize_kwargs
    ) -> "QuantizedTensor":
        """
    ...
    @classmethod
    def _dequant_and_fallback(
        cls, func: Callable, args: tuple, kwargs: dict
    ) -> Any:
...
def fp8_linear(func: Callable, args: tuple, kwargs: dict) -> Any:
    ...
    if isinstance(input_tensor, QuantizedTensor) and isinstance(
        ...
        plain_input, scale_a = TensorCoreFP8Layout.get_plain_tensors(
            input_tensor
        )
        ...
        try:
            ...
            logger.warning(
                f"FP8 _scaled_mm failed: {e}, falling back to dequant"
            )
...
def fp8_transpose(func: Callable, args: tuple, kwargs: dict) -> Any:
    ...
    if isinstance(input_tensor, QuantizedTensor):
        ...
        return QuantizedTensor(
            new_qdata, input_tensor._layout_type, input_tensor._layout_params
        )
    return func(*args, **kwargs)
...
def fp8_view(func: Callable, args: tuple, kwargs: dict) -> Any:
    ...
    if isinstance(input_tensor, QuantizedTensor):
        ...
        new_qdata = (
            input_tensor._qdata.view(*new_shape)
            if isinstance(new_shape, (list, tuple))
            else input_tensor._qdata.view(new_shape)
        )
        return QuantizedTensor(
            new_qdata, input_tensor._layout_type, input_tensor._layout_params
        )
...
class FP8Linear(nn.Module):
    ...
    for compatibility with existing tensor-loading code.
    """
    ...
    def __init__(
        ...
        if bias:
            self.register_buffer(
                "_bias",
                torch.zeros(
                    out_features, device=device, dtype=self.compute_dtype
                ),
            )

    @property
    def weight(self) -> Optional[QuantizedTensor]:
        """Get the FP8 quantized weight."""
    ...
    def bias(self) -> Optional[torch.Tensor]:
        """Get bias tensor."""
        return (
            self._bias if self._has_bias and hasattr(self, "_bias") else None
        )
...
class UnscaledFP8Linear(nn.Module):
    ...
    def __init__(
        ...
        if bias:
            self.register_buffer(
                "bias",
                torch.zeros(out_features, device=device, dtype=compute_dtype),
            )
        else:
            self.register_buffer("bias", None)

    def set_weight(
        self, fp8_weight: torch.Tensor, bias: Optional[torch.Tensor] = None
    ):
    ...
    def extra_repr(self) -> str:
        ...
        Includes basic shape information for quick inspection in debug logs.
        """
