...
class ModelOptimizer:
    ...
    def detect_format(self, model_path: str) -> ModelFormat:
        ...
        if path.is_dir():
            ...
            onnx_files = list(path.glob("*.onnx")) + list(
                path.glob("**/*.onnx")
            )
            if onnx_files:
                return ModelFormat.ONNX
            ...
            # Check for PyTorch
            pytorch_files = list(path.glob("*.bin")) + list(
    ...
    def get_gguf_path(
        ...
        return (
            str(path.parent / model_name / gguf_filename)
            if path.is_dir()
            else str(path.parent / gguf_filename)
        )
    ...
    def convert_to_gguf(
        ...
        try:
            self.logger.info(
                f"Converting {model_path} to GGUF ({quantization})..."
            )
            ...
            cmd = [
                ...
                output_path,
                "--outtype",
                self._quantization_to_outtype(quantization),
            ]
    ...
    def ensure_gguf(
        ...
        # Try to convert
        success, gguf_path, error = self.convert_to_gguf(
