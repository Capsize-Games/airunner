...
def detect_model_dtype(model_path: str) -> str:
    ...
    logger.debug(
        f"Could not detect dtype for {model_path}, defaulting to bfloat16"
    )
    return "bfloat16"
...
def get_model_info(model_path: str) -> Dict[str, Any]:
    ...
    """
    from airunner_services.utils.vram_utils import get_model_file_size_gb
    ...
    return {
        "path": model_path,
        "native_dtype": (
            detect_model_dtype(model_path) if path.exists() else "bfloat16"
        ),
