...
def safe_extract_zip(zip_file: ZipFile, dest_dir: Union[str, Path]) -> None:
    ...
    for member in zip_file.infolist():
        ...
        if name.startswith(('/', '\\')):
            raise UnsafeZipPathError(f"Unsafe zip entry (absolute path): {name}")
        ...
        if any(part == '..' for part in parts):
            raise UnsafeZipPathError(f"Unsafe zip entry (path traversal): {name}")
        ...
        if not str(target_path).startswith(str(dest_path) + os.sep) and target_path != dest_path:
            raise UnsafeZipPathError(f"Unsafe zip entry (escapes dest): {name}")
