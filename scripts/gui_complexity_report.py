...
DEFAULT_ROOT = Path("src/airunner")
DEFAULT_OUTPUT = Path("build/gui_complexity")
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Report complexity hotspots in GUI code.",
    )
    parser.add_argument("--path", type=Path, default=DEFAULT_ROOT)
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument(
        "--exclude", default=",".join(DEFAULT_EXCLUDES),
    )
...
def count_code_lines(node: ast.AST, lines: list[str]) -> int:
    ...
    ignored = (
        set(range(skipped[0], skipped[1] + 1)) if skipped else set()
    )
    count = 0
    for lineno in range(
...
def collect_size_entries(
    ...
    return sorted(
        entries, key=lambda item: (-item["line_count"], item["lineno"]),
    )


def syntax_error_report(
    path: Path, root: Path, exc: SyntaxError,
) -> dict[str, Any]:
...
def build_file_report(
    ...
    complex_blocks = [
        block
        for block in blocks
        if rank_exceeds(block["rank"], thresholds.max_complexity_rank)
    ]
    return {
        "path": path.relative_to(root).as_posix(),
        ...
        "max_complexity": max(
            (block["complexity"] for block in blocks), default=0,
        ),
        "max_rank": max(
            (block["rank"] for block in blocks), default="A",
        ),
...
def report_score(report: dict[str, Any], thresholds: Thresholds) -> int:
    ...
    score = (
        len(report["long_functions"]) * 3
        + len(report["long_classes"]) * 4
    )
...
def summarize_reports(
    ...
    return {
        ...
        "files_over_limit": sum(
            report["file_lines"] > thresholds.max_file_lines
            for report in reports
        ),
        "long_functions": sum(
            len(report["long_functions"]) for report in reports
        ),
        "long_classes": sum(
            len(report["long_classes"]) for report in reports
        ),
        "complex_blocks": sum(
            len(report["complex_blocks"]) for report in reports
        ),
        "low_mi_files": sum(
            report["mi"] < thresholds.min_mi for report in reports
        ),
        "hotspot_files": sum(
            report_score(report, thresholds) > 0 for report in reports
        ),
    }


def area_key(path: str) -> str:
    """Group reports into broad GUI subsystems."""
...
def issue_reasons(
    ...
    if report["file_lines"] > thresholds.max_file_lines:
        reasons.append(
            f"{report['file_lines']} SLOC "
            f"(> {thresholds.max_file_lines})",
        )
    if report["long_functions"]:
        reasons.append(
            f"{len(report['long_functions'])} function(s) "
            f"> {thresholds.max_function_lines} lines",
        )
    if report["long_classes"]:
        reasons.append(
            f"{len(report['long_classes'])} class(es) "
            f"> {thresholds.max_class_lines} lines",
        )
    if report["complex_blocks"]:
        reasons.append(
            f"{len(report['complex_blocks'])} block(s) "
            f"above rank {thresholds.max_complexity_rank}",
        )
    if report["mi"] < thresholds.min_mi:
        reasons.append(
            f"maintainability index {report['mi']} "
            f"(< {thresholds.min_mi})",
        )
    return reasons
...
def build_issue_candidate(
    ...
    """Build one draft issue candidate."""
    area_score = sum(
        report_score(report, thresholds) for report in reports
    )
    top_reports = sorted(
        reports,
        key=lambda item: -report_score(item, thresholds),
    )[:5]
    lines = [
        "## Why",
        f"The `{area}` area exceeds the current GUI complexity targets.",
        "",
        "## Targets",
    ...
    return {
        "area": area,
        "score": area_score,
        "title": f"Refactor: Reduce GUI complexity in {area}",
        "body_markdown": "\n".join(lines),
...
def filter_xenon_output(text: str) -> str:
    ...
    lines = [
        ...
        if "RequestsDependencyWarning" not in line
    ]
...
def build_markdown(payload: dict[str, Any]) -> str:
    ...
    lines += [
        f"- {item['title']}" for item in payload["issue_candidates"]
    ]
    lines += [
        "",
        "## Xenon Gate",
        payload["xenon"]["output"] or "No Xenon findings.",
    ]
    return "\n".join(lines) + "\n"
...
def write_outputs(
    ...
    json_path = output_dir / "gui_complexity_report.json"
    md_path = output_dir / "gui_complexity_report.md"
...
def main() -> int:
    ...
    reports = [
        build_file_report(path, root, thresholds)
